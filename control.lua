...
if script.active_mods["warptorio2_expansion"] then
    ...
    script.on_nth_tick(60, function(event)
        ...
        for _, surface in pairs(game.surfaces) do
            local entities = surface.find_entities_filtered { area = { { -100, -100 }, { 100, 100 } }, name = "wpe_broken_lab" }
            if #entities > 0 and check_container_for_items(entities[1], fix_items) then
